...
/// <summary>
/// 実行パフォーマンス計測: 各文法パターンの Parse (字句解析+構文解析+AST 構築) と Tokenize (字句解析のみ)。
/// [Params] で入力サイズを Small/Medium/Large に切り替え、規模に対するスケールを見る。
/// </summary>
[MemoryDiagnoser]
public class ParseTokenizeBenchmarks
{
    [Params(BenchSize.Small, BenchSize.Medium, BenchSize.Large)]
    public BenchSize Size { get; set; }

    private string? _deepPrec, _wideRules, _manyTokens, _deepNest, _megaLang;
    ...
    public void Setup()
    {
        ...
        Console.WriteLine($"# Input size ({Size}): " +
            ...
            $"ManyTokens={_manyTokens.Length:N0}, DeepNest={_deepNest.Length:N0}, MegaLang={_megaLang.Length:N0}");
    ...
    // --- Tokenize: 字句解析のみ (Parser が内部で呼ぶ Lexer.Tokenize を独立計測) ---
