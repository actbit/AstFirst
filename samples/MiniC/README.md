...
**JA:** 軽量C言語サンプル。変数宣言・代入・`print`・`if`/`while`・ブロック文・`bool` リテラル。Generator 生成の `ProgramListener` を継承した意味解析（スコープ管理・シンボル解決・型チェック）のデモ。
**EN:** Lightweight C sample. Variables, assignment, `print`, `if`/`while`, blocks, `bool` literals. Demo of semantic analysis (scope management, symbol resolution, type checking) on a generator-emitted `ProgramListener`.
...
`SemanticAnalyzer` は `ProgramListener` を継承し、`Walk` で次を検出する:
