...
**JA:** 軽量C言語サンプル。変数宣言・代入・`print`・`if`/`while`・ブロック文・`bool` リテラル。ルート `Program` クラスの `[Enter]`/`[Exit]` 属性による意味解析（スコープ管理・シンボル解決・型チェック）のデモ。
**EN:** Lightweight C sample. Variables, assignment, `print`, `if`/`while`, blocks, `bool` literals. Demo of semantic analysis (scope management, symbol resolution, type checking) via `[Enter]`/`[Exit]` attributes on the root `Program` class.
...
ルート `Program` クラスの `[Enter]`/`[Exit]` 属性メソッド（Generator が Parse 後に Walker で呼ぶ）から `SemanticAnalyzer` の各メソッドが呼ばれ、次を検出する:
