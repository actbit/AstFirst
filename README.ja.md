...
| 生成タイミング | コンパイル時 (Source Generator) | ビルド時コード生成 | 実行時 (解釈実行) | 該当なし |
| アルゴリズム | LALR(1) テーブル駆動 | LL(\*) / ALL(\*) | 再帰下降コンビネータ | 該当なし |
| 対象言語 | 任意の DSL / 言語 | 任意の言語 | 任意の言語 | C#/VB のみ |
| エラー回復 | panic mode | あり | 限定的 | 該当なし |

**なぜ作ったか**: いつも書いているのと同じ C# だけで文法を定義し（別の `.g4` ファイル不要・IDE のリファクタリング支援もそのまま）、コンパイル時に高速なテーブル駆動パーサを生成し（実行時コード生成なし・AOT 対応）、意味解析ヘルパ（スコープ付きシンボル表・2パス目 Walker・型システム・`[Enter]`/`[Exit]`/`[OnReduce]` 属性ルール）を組み込みの AST として得るため。小さな言語や DSL が、ツールチェインではなく単一プロジェクトで済む。
