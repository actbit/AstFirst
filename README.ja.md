...
**なぜ作ったか**: いつも書いているのと同じ C# だけで文法を定義し（別の `.g4` ファイル不要・IDE のリファクタリング支援もそのまま）、コンパイル時に高速なテーブル駆動パーサを生成し（実行時コード生成なし・AOT 対応）、意味解析ヘルパ（スコープ付きシンボル表・2パス目 Walker・型システム・`[Enter]`/`[Exit]`/`[OnReduce]` 属性ルール）を組み込みの AST として得るため。
